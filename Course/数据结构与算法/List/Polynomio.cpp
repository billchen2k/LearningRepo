...
void Polynomio::polyProduct(Polynomio p, double co, int de)
{
    ...
    {
        term[i].setValue(p.term[i].degree + de, p.term[i].coefficient * co);
    }
    Simplify();
}
...
void Polynomio::write()
{
    ...
    totalDegree = coMax;
    cout << "接下来请按照降幂顺序，依次输入 x^" << coMax << " 到 x^0 的系数，空格分隔：" << endl;
...
void Polynomio::read()
{
    for (int i = totalDegree; i >= 0; i--)
    {
        if (i == totalDegree)
        {
            printf("%.2fx^%d ", term[i].coefficient, term[i].degree);
        }
        else
        {
            if (term[i].coefficient >= 0)
...
class StackNode
{
  public:
    ...
    void top(Polynomio &dstPoly)
    {
        dstPoly = topnode->entry;
    }
    void clear()
    {
        topnode = NULL;
