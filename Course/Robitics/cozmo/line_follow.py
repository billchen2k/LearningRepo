...
def get_cnt_rects(img, zn):
    ...
    # cnt_img, cnts = cv2.findContours(inv_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def cozmo_run_with_viewer(program):
...
def main(argv):
    ...
    if usecase == 'cli':
        cozmo.run_program(cozmo_cli, use_3d_viewer=True, use_viewer=True)
    elif usecase == 'step_forward':
        cozmo.run_program(cozmo_step_forward, use_3d_viewer=True, use_viewer=True)
