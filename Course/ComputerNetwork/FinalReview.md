...
- 功能组成 - 通信子网（OSI下三层通信子网）、资源子网网（上三层的处理）
  - ![image-20200106161512400](FinalReview.assets/image-20200106161512400.png)
...
- 按照拓补结构分：
  - ![image-20200106161659522](FinalReview.assets/image-20200106161659522.png)
- 按传输方式：广播式、点对点
...
- 通信过程
  - ![image-20200106170513123](FinalReview.assets/image-20200106170513123.png)
...
- 应用层、传输层、网际层、网络接口层：
  ...
  - ![image-20200106175751387](FinalReview.assets/image-20200106175751387.png)
...
- 一个典型的数据通信模型：
  ...
  ![image-20200106184834304](FinalReview.assets/image-20200106184834304.png)
...
- 带宽：表示在单位时间内从网络的某一点到另一点所能通过的**最高数据率**（理想中的）
  - Eg. ![image-20200106191256144](FinalReview.assets/image-20200106191256144.png)
...
- 数字数据编码为数字信号：
  - ![image-20200106195313261](FinalReview.assets/image-20200106195313261.png)
  - NRZ - 非归零编码（编码容易实现，没有检错功能，需要建立一个信道确立时钟周期
    - <u>1 传输完之后不归零</u>
  - 不归零逆转编码 —— 对于全 0 没有问题，对于全 1 有问题
  - **曼彻斯特编码**：自同步的编码方式
    - 数据传输速率只有调制速率的1/2
    - ![img](FinalReview.assets/1364699-20190527165214968-1729554105.jpg)
...
- 几种方法：
  - 字符计数法：帧首部使用一个计数字段来表明字符数
    - ![image-20200106213543772](FinalReview.assets/image-20200106213543772.png)
...
- 检验编码
  ...
  - CRC 循环冗余码
    ...
    - 冗余码的计算：**在原数据之后加 r 个 0**，使用**模 2 除法**，在竖式计算中使用的是异或的方式做减，最后的余数作为冗余码，将冗余码加到刚刚补的 0 后面一起传送
      - Eg.![image-20200106215400534](FinalReview.assets/image-20200106215400534.png)
...
- 纠错编码 - 海明码 Hamming Code
  ...
  - 计算方式：校验位的目标：所要校验的位异或 = 0。校验位为第 2^n^ 位。
    - ![image-20200106222247752](FinalReview.assets/image-20200106222247752.png)
...
- 关于协议
  ...
  - 研究的前提：仅讨论单工通信
- 协议细节：
  - ![image-20200106224813296](FinalReview.assets/image-20200106224813296.png)
...
- 协议细节：
  - ![image-20200106225559471](FinalReview.assets/image-20200106225559471.png)
...
- 协议中的窗口：
  ...
  - ![image-20200106230132206](FinalReview.assets/image-20200106230132206.png)
- 协议细节：
  - ![image-20200106225949892](FinalReview.assets/image-20200106225949892.png)
...
- **ALOHA 协议**（以下为**随机访问介质访问控制**）
  - 纯 ALOHA 协议：随机重新发送，不按时间槽发送
    - ![image-20200106232642474](FinalReview.assets/image-20200106232642474.png)
...
- 拓补结构：![image-20200107004512298](FinalReview.assets/image-20200107004512298.png)
- 局域网介质访问控制的方法
...
- **以太网 MAC 帧**：（V2格式）
  - ![image-20200107005655456](FinalReview.assets/image-20200107005655456.png)
  - 数据段含义：
    - 目的地址：如果是全 f 则为广播地址
    - 类型：数据的协议类型
...
- 802.11 的 MAC 帧头格式：
  - ![image-20200107010106922](FinalReview.assets/image-20200107010106922.png)
...
- 源主机报文切割为等大的分隔片段，依次发送到交换设备中
  - ![image-20200107012201390](FinalReview.assets/image-20200107012201390.png)
- 相对于报文交换，<u>存储管理更容易</u>，线路的可靠性也有所提高（报文变短）
- 缺点：乱序到达，需要增大排序开销
- 上述三种方式的比较：![image-20200107012512774](FinalReview.assets/image-20200107012512774.png)
...
- 和数据报的对比：![image-20200107013213404](FinalReview.assets/image-20200107013213404.png)
...
- 和相邻路由器交换信息（直接交换路由表）
  - 每隔 30s 都会刷新一次
- 一个示例：
  - ![image-20200107023947676](FinalReview.assets/image-20200107023947676.png)
...
- ![image-20200107014020639](FinalReview.assets/image-20200107014020639.png)
  ...
  - 分组相关：
    ...
    - Eg.![image-20200107015220077](FinalReview.assets/image-20200107015220077.png)
...
- 分类的 IP 地址：
  ...
  - ![image-20200107015742460](FinalReview.assets/image-20200107015742460.png)
  - 一些特殊 IP 地址：
    - 全为 0，表示默认路由
    - 全为 1：本网广播地址
  ...
  - 私有 IP 地址：
    - ![image-20200107020303236](FinalReview.assets/image-20200107020303236.png)
  - 各类地址可以划分的网络个数：
    - ![image-20200107020320805](FinalReview.assets/image-20200107020320805.png)
