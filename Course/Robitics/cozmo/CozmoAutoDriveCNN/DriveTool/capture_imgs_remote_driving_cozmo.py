...
class RemoteControlCozmo:
    ...
    def handle_key(self, key_code, is_shift_down, is_ctrl_down, is_alt_down, is_key_down):
        ...
        ## capture a pic from cozmo into disk wich a driving command key 
        ## has beeing pressed
        if self.drive_forwards or self.drive_back or self.turn_right or self.turn_left :
            print("be press")
    ...
    def update_driving(self):
        ...
        if l_wheel_speed != 0 or r_wheel_speed != 0 :
            screen  = np.array(self.cozmo.world.latest_image.raw_image)
            
            file_name = str(time.time()) + "_"+ str(l_wheel_speed)+"_"+str(r_wheel_speed)+".jpg"
            cv2.imwrite('./TestImgv2/'+file_name, screen)
