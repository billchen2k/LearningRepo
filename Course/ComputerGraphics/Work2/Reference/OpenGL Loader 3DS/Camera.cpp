...
//	char buf[128];
	wsprintf(g_strText[1],"Position:%d %d %d LookAt:%d %d %d",int(m_vPosition.x),int(m_vPosition.y),int(m_vPosition.z),int(m_vView.x),int(m_vView.y),int(m_vView.z));
//	if(m_pText)
//		m_pText->EditItem(m_nTextID,0.0f,1.0f,0.0f,buf);
